...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoch',type=int, default=10, help='total training epoches')
    parser.add_argument('--seed',type=int, default=2, help='seed')
    parser.add_argument('--weight_d',type=float, default=0.00001, help='weight decay for regularization')
    parser.add_argument('--save_path',type=str, default='Graphs', help='saved path of each setting')
    parser.add_argument('--data_path',type=str, default='P1.pt', help='saved path of input data')
    ...
    #pretrained_dict = torch.load("vgg11_DC.pth")
    ...
    x = np.linspace(min(index_train), max(index_train), 300) 
    spl_T = make_interp_spline(index_train, Train_Accuracy, k=3)  # type: BSpline
    spl_V = make_interp_spline(index_train, Valid_Accuracy, k=3)  # type: BSpline
    y_T = spl_T(x)
    y_V = spl_V(x)
    weight = 0.9
    ...
    print('File:{}, Setting: Seed:{}, Epoch: {}, Batch size: {}, Learning rate: {:.6f}, Weight decay: {}, gpu:{}, Saved path: {}'.format(
        args.data_path, args.seed, args.epoch, args.b, args.lr, args.weight_d, args.gpu, args.save_path),
        file=f)
    ...
    with torch.no_grad():
        ...
        Class_labels = ['natural sounds','alarm calls']
